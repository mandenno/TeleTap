...
static const int gpsRX = 5, gpsTX = 6;
static const uint32_t gpsBaud = 9600;
static const int simRX = 2, simTX = 4;
...
String ipAddress = "www.fastagas.com/teletap"; //this is the ip address of our server - e.g. "123.123.123.123"
String APN = "safaricom"; //check your Internet provider's website for the APN e.g. "internet"
...
int sim800Pin = 7; //error pin
int gpsPin = 8; //error pin
...
void loop() {
  alarm_status = digitalRead(buzzerPin);
  current = millis();
  ...
  if (elapsed >= (refreshRate * 1000) && alarm_status = HIGH) {
    beep(50); //Beep
    beep(50); //Beep
    delay(1000);
    sendData();
...
void sendLocation(String lat, String lon) {
  ...
  Sim800l.println("AT+HTTPPARA=\"URL\", \"https://" + ipAddress +  "/log_info.php?dev_id=13&lat=" + lat + "&lon=" + lon + "\"");
  waitUntilResponse("OK");
  //GO
